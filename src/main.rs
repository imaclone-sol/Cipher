...
fn encrypt_text(plaintext: &str, key: &[u8], iv: &[u8]) -> Vec<u8> {
    let aead = Aes256Gcm::new_from_slice(key).expect("Failed to create AEAD instance");
    let ciphertext = aead.encrypt(iv.into(), plaintext.as_bytes().as_ref()).expect("Encryption failed");
    ciphertext
}

fn decrypt_text(ciphertext: &[u8], key: &[u8], iv: &[u8]) -> Option<String> {
    let aead = Aes256Gcm::new_from_slice(key).expect("Failed to create AEAD instance");
    match aead.decrypt(iv.into(), ciphertext.as_ref()) {
        Ok(decrypted) => Some(String::from_utf8(decrypted).unwrap_or_else(|_| String::new())),
        Err(_) => None,
    }
}
